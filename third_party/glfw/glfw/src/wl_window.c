...
{
    // vmxplore patch (see ../../../VMXPLORE-PATCH.md): a NULL output asks
    // the compositor to fullscreen on the output the surface is currently
    // on. Upstream names window->monitor->wl.output, and the only monitor
    // a client can name under Wayland is a guess (GLFW's "primary", the
    // first output announced), so a window on any other head jumped
    // across the desktop on every fullscreen toggle.
